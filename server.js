...
// ⭐ Serve PUBLIC folder (upload system)
app.use(express.static("public"));

// ⭐ Serve ACCOUNTS folder (frontpage + login UI)
app.use("/accounts", express.static("accounts"));

// ⭐ Import backend.js (account system API)
...
function loadWebsites() {
  if (!fs.existsSync(WEBSITES_FILE)) fs.writeFileSync(WEBSITES_FILE, "[]");
...
app.post("/upload", upload.single("file"), (req, res) => {
  ...
  if (!userEntry) {
    userEntry = { Username: username, urls: [] };
...
app.post("/unlock/:id", (req, res) => {
  ...
  if (!data[id]) return res.json({ success: false, message: "Invalid link" });
...
app.get("/download/:id", (req, res) => {
  ...
  if (!data[id]) return res.send("Invalid link");
  if (!data[id].unlocked) return res.send("You must unlock the file first");
